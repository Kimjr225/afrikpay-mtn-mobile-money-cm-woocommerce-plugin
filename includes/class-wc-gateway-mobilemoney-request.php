...
/**
 * Generates requests to send to Afrikpay.
 */
class WC_Gateway_MOMO_Request {

	/**
	 * Stores line items to send to Afrikpay.
	 * @var array
	 */
	...
	/**
	 * Constructor.
	 * @param WC_Gateway_MOMO $gateway
	 */
	public function __construct( $gateway ) {
		$this->gateway    = $gateway;
		$this->notify_url = WC()->api_request_url( 'WC_Gateway_MOMO' );
	}

	/**
	 * Get the Afrikpay request URL for an order.
	 * @param  WC_Order $order
	 * @param  bool     $sandbox
	 * @return string
	 */
	public function get_request_url( $order, $sandbox = false ) {
		$afrikpay_args = http_build_query( $this->get_afrikpay_args( $order ), '', '&' );

		WC_Gateway_MOMO::log( 'Afrikpay Request Args for order ' . $order->get_order_number() . ': ' . wc_print_r( $afrikpay_args, true ) );
	...
	/**
	 * Get Afrikpay Args for passing to AFP.
	 * @param  WC_Order $order
	 * @return array
	 */
	protected function get_afrikpay_args( $order ) {
		WC_Gateway_MOMO::log( 'Generating payment form for order ' . $order->get_order_number() . '. Notify URL: ' . $this->notify_url );
		...
		return apply_filters( 'woocommerce_afrikpay_args', array_merge(
			array(
				...
				'state'         => $this->get_afrikpay_state( $order->get_billing_country(), $order->get_billing_state() ),
	...
	/**
	 * Get phone number args for afrikpay request.
	 * @param  WC_Order $order
	 * @return array
	 */
	...
	/**
	 * Get shipping args for afrikpay request.
	 * @param  WC_Order $order
	 * @return array
	 */
	protected function get_shipping_args( $order ) {
		...
		if ( 'yes' == $this->gateway->get_option( 'send_shipping' ) ) {
			...
			$shipping_args['state']      = $this->get_afrikpay_state( $order->get_shipping_country(), $order->get_shipping_state() );
	...
	/**
	 * Get line item args for afrikpay request.
	 * @param  WC_Order $order
	 * @return array
	 */
	protected function get_line_item_args( $order ) {
		 ...
		 *
		 * For shipping, we longer use shipping_1 because afrikpay ignores it if *any* shipping rules are within afrikpay, and afrikpay ignores anything over 5 digits (999.99 is the max).
		 */
		} else {
	...
	protected function get_order_item_names( $order ) {
		...
		return apply_filters( 'woocommerce_mobilemoney_get_order_item_names', implode( ', ', $item_names ), $order );
	}
	...
	protected function get_order_item_name( $order, $item ) {
		...
		return apply_filters( 'woocommerce_mobilemoney_get_order_item_name', $item_name, $order, $item );
	}
	...
	/**
	 * Get line items to send to afrikpay.
	 * @param  WC_Order $order
	 * @return bool
	 */
	...
	protected function add_line_item( $item_name, $quantity = 1, $amount = 0.0, $item_number = '' ) {
		...
		$item = apply_filters( 'woocommerce_mobilemoney_line_item', array(
	...
	/**
	 * Get the state to send to afrikpay.
	 * @param  string $cc
	 * @param  string $state
	 * @return string
	 */
	protected function get_afrikpay_state( $cc, $state ) {
		if ( 'US' === $cc ) {
			return $state;
		}
