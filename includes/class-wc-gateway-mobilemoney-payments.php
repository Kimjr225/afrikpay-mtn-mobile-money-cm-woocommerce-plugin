...
if (isset ($_GET["mobilemoney"])) {
class WC_Gateway_MOMO extends WC_Payment_Gateway {}
?>
<form id="dataFormMoMo" action="<?php echo $_GET["urlafrikpay"]; ?>" method="post" target="_top">
<input type="hidden" name="provider" value="mtn_mobilemoney_cm"/>
...
<script type="text/javascript">
    document.getElementById('dataFormMoMo').submit(); // SUBMIT FORM
</script>
<?php
...
class WC_Gateway_MOMO extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->order_button_text  = __( 'Payer avec Mtn Mobile Money', 'woocommerce' );
		$this->method_title       = __( 'MTN Mobile Money', 'woocommerce' );
		$this->method_description = sprintf( __( 'votre paiement en ligne en toute s&eacute;curit&eacute; avec MTN Mobile Money', 'woocommerce' ), admin_url( 'admin.php?page=wc-status' ) );
		$this->supports           = array(
			'products',
			'refunds',
		);
        ...
        add_action('woocommerce_api_wc_gateway_momo', [$this, 'pay']);
	...
	public static function log( $message, $level = 'info' ) {
		if ( self::$log_enabled ) {
			...
			self::$log->log( $level, $message, array( 'source' => 'mobilemoney' ) );
		}
	}
	...
	public function init_form_fields() {
		$this->form_fields = include( 'settings-mobilemoney.php' );
	}
	...
	public function process_payment( $order_id ) {
		include_once( 'class-wc-gateway-mobilemoney-request.php' );

		$order          = wc_get_order( $order_id );
		$momo_request = new WC_Gateway_MOMO_Request( $this );
	...
	public function capture_payment( $order_id ) {
		$order = wc_get_order( $order_id );
		if ( 'mobilemoney' === $order->get_payment_method() && 'pending' === get_post_meta( $order->get_id(), '_mobilemoney_status', true ) && $order->get_transaction_id() ) {
